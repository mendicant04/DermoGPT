...
Evaluate with these steps (format-agnostic; focus on content):
A) Extraction (be robust even if the candidate is unstructured):
   - From GROUND_TRUTH_RAW, extract:
     gt_reasoning: inside <reasoning>...</reasoning> if present; else best-effort summary.
     gt_final_dx: inside <final_diagnosis>...</final_diagnosis> if present; else best-effort label.
   - From CANDIDATE_RAW, extract:
     cand_reasoning: the explanation/rationale (anywhere).
     cand_final_dx: the single most likely final diagnosis term/phrase (prefer lines starting with 'Final', 'Diagnosis', or the conclusive statement at the end).

B) Reasoning Alignment:
   - Decompose gt_reasoning into ≤25 atomic claims (morphology, distribution, logic, differential cues).
   - For each claim, label wrt cand_reasoning: Supported | PartiallySupported | Contradicted | Missing | Vague.
   - Compute:
     ...
     reasoning_score [0-100] = round(100 * max(0, recall_like - 0.5*precision_penalty), 1)

C) Diagnosis Similarity (graded, not binary):
   - Normalize synonyms (e.g., 'tinea corporis' ≈ 'ringworm'; 'urticaria' ≈ 'hives'). Consider hierarchy and clinical proximity.
   - Decide semantic relation between cand_final_dx and gt_final_dx:
     Exact | Synonym | Parent | Child | Sibling/CloseDifferential | SameSuperfamily | UnrelatedPlausible | WrongSystem | Nonsense/NoAnswer.
   - Map to similarity in [0,1] (guideline):
     Exact/Synonym=1.0; Parent/Child=0.85; Sibling/CloseDifferential=0.7; SameSuperfamily=0.5;
     UnrelatedPlausible=0.3; WrongSystem=0.1; Nonsense/NoAnswer=0.0.
   - diagnosis_score [0-100] = round(100 * similarity, 1)

D) Overall:
   - overall [0-100] = round(0.5 * reasoning_score + 0.5 * diagnosis_score, 1)

Also provide a concise feedback (≤50 words).
...
SYSTEM_PROMPT_IMAGE = (
    ...
    "You will be given an image and two texts: REFERENCE (GT) and CANDIDATE. "
...
def agg_text_votes(vs: List[dict]) -> dict:
    n = max(1, len(vs))
    overall = round(sum(float(v.get("overall", 0.0)) for v in vs) / n, 2)
    ...
    reason = sum(_safe_get(v, ["reasoning", "score"]) for v in vs) / n
    dx = sum(_safe_get(v, ["diagnosis", "score"]) for v in vs) / n
    dx_sim = sum(_safe_get(v, ["diagnosis", "similarity"]) for v in vs) / n


    keys = ["supported", "partial", "contradicted", "missing", "vague", "extra_incorrect", "total_ref_claims"]
    counts = {}
    for k in keys:
        counts[k] = round(sum(_safe_get(v, ["reasoning", "counts", k]) for v in vs) / n)


    r_keys = ["reasoning_alignment", "diagnosis_similarity", "internal_consistency"]
    rubric = {}
    for rk in r_keys:
        rubric[rk] = round(sum(_safe_get(v, ["rubric", rk]) for v in vs) / n, 3)

    fb = " | ".join(v.get("short_feedback", "") for v in vs if v.get("short_feedback"))[:240]

    return {
        "overall": overall,
        "reasoning_score": round(reason, 2),
        "diagnosis_score": round(dx, 2),
...
def run_eval(gt_path: Path, pred_path: Path, provider: ApiProvider,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in concurrent.futures.as_completed(futs):
            ...
            try:
                ...
                rows.append({
                    "id": _id,
                    "reasoning_score": rec["text_aggregate"]["reasoning_score"],
                    "diagnosis_score": rec["text_aggregate"]["diagnosis_score"],
                    "dx_similarity": rec["text_aggregate"]["dx_similarity"],
                    "overall_text": rec["text_aggregate"]["overall"],
                    "final_overall": rec["final_overall"],
                    "reasoning_alignment": rec["text_aggregate"]["rubric"]["reasoning_alignment"],
                    "diagnosis_similarity": rec["text_aggregate"]["rubric"]["diagnosis_similarity"],
                    "internal_consistency": rec["text_aggregate"]["rubric"]["internal_consistency"],
    ...
    macro = {
        ...
        "n_samples": len(rows),
        "mean_reasoning_score": mean("reasoning_score"),
        "mean_diagnosis_score": mean("diagnosis_score"),
...
def main():
    ...
    ap.add_argument("--out_dir", default="./eval_task3/results")
