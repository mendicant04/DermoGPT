...
def run_inference_on_item(item, image_base_path, generation_args, provider: ApiProvider):
    ...
    if not image_rel:
        print(
            f"[{device}] Warning: {item.get('id')} missing image field, skipping.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        for turn in item.get("conversations", []):
            if turn.get("from") == "human":
                out_item["conversations"].append(
                    {"from": "human", "value": turn.get("value", "")}
                )
                break
        ...
        print(
            f"[{device}] Critical error while processing ID {item.get('id')}: {e}",
            file=sys.stderr,
        )
        return None
...
def worker_proc(
    ...
    print(
        f"[Worker {worker_idx} on {device}]: Provider ready, pending {len(tasks)} items.",
        flush=True,
    )
    ...
    for item in tqdm(
        ...
        if processed_ids is not None and _id in processed_ids:
            continue
        out_item = run_inference_on_item(item, image_base_path, generation_args, provider)
        if out_item is None:
            continue
        ...
        for i in range(len(chunks)):
            job_args.append(
                (
                    chunks[i],
                    i,
                    devices[i] if i < len(devices) else args.device,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json-files",
        type=str,
        default="task3_2.jsonl, task1_2.jsonl, task1_1.jsonl, task3_1.jsonl",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of parallel processes for concurrent API calls",
    )
    parser.add_argument(
        "--output-suffix",
        type=str,
    ...
    parser.add_argument(
        ...
        help="API base URL for an OpenAI-compatible endpoint",
    )
    parser.add_argument(
        "--api_key",
        type=str,
