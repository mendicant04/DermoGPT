...
import argparse, warnings, json, os, sys, torch, math, multiprocessing, re, hashlib, random
...
def best_match_in_options(target, options, jaccard_thr=0.35):
    ...
    for opt in options:
        o_tok = _tokens(opt)
        if not o_tok: continue
        inter = len(t_tok & o_tok); uni = len(t_tok | o_tok)
        s = inter / (uni + 1e-9)
        if s > best_s:
            best_s, best = s, opt
...
def build_question(kind, level_idx, parent_category, options, sample_id):
    ...
    if kind == "top":
        ...
        tmpl = stable_shuffle(SUB_LEVEL_PROMPTS_GEN, f"sub-{sample_id}-{level_idx}")[0]
        q = tmpl.format(parent_category=parent_category, options_list=format_options(options_shuffled))
        return q, options_shuffled
    else:  # final
        tmpl = stable_shuffle(FINAL_LEVEL_PROMPTS_GEN, f"final-{sample_id}-{level_idx}")[0]
        q = tmpl.format(parent_category=parent_category, options_list=format_options(options_shuffled))
        return q, options_shuffled
    ...
    while depth < max_depth:
        ...
        if canonical_in_items:
            ...
            if len(leaf_options) == 1:
                ...
                # out_item["conversations"].append({"from": "human", "value": f"Following the diagnostic path to '{parent_category}', the evidence points to a single definitive diagnosis: {leaf_options[0]}."})
                # out_item["conversations"].append({"from": "gpt", "value": leaf_options[0]})
                break

            level_idx = len(true_path)
            kind = "final"
            q_text, options_ordered = build_question(
                kind=kind, level_idx=level_idx, parent_category=parent_category,
                options=leaf_options, sample_id=item.get("id","")
            )
            gold_label = best_match_in_options(canonical_dx, options_ordered) or options_ordered[0]
            ...
            if not sub_categories:
                ...
                if items:
                    ...
                    q_text, options_ordered = build_question(
                        kind=kind, level_idx=level_idx, parent_category=parent_category,
                        options=items, sample_id=item.get("id","")
                    )
                    gold_label = best_match_in_options(canonical_dx, options_ordered) or options_ordered[0]
                else:
                    break
                ...
                gold_exact = best_match_in_options(next_cat, sub_categories) or sub_categories[0]

                kind = "top" if first_turn else "sub"
                q_text, options_ordered = build_question(
                    kind=kind, level_idx=depth, parent_category=parent_category,
                    options=sub_categories, sample_id=item.get("id","")
                )
                gold_label = best_match_in_options(gold_exact, options_ordered) or options_ordered[0]
        ...
        if first_turn:
            ...
            convo_for_model.append({"role": "user", "content": [{"type": "text", "text": q_text}]})
...
def main(args):
    ...
    shared_jsonl = os.path.join(output_dir, f"{base}_{loaded_model_name}_hierarchical.jsonl")
    out_array   = os.path.join(output_dir, f"{base}_{loaded_model_name}_hierarchical_infer.json")
    metric_path = os.path.join(output_dir, f"{base}_{loaded_model_name}_hierarchical_metrics.json")
